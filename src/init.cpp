...
std::string LicenseInfo()
{
    ...
    return CopyrightHolders(strprintf(_("Copyright (C) %i-%i").translated, 2022, COPYRIGHT_YEAR) + " ") +
           ...
           strprintf(_("\nThe block explorer is available from %s."),
               URL_BLOCK_EXPLORER) +
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    for (const auto& arg : args.GetUnsuitableSectionOnlyArgs()) {
        errors += strprintf(_("Config setting for %s only applied on %s network when in [%s] section.\n"), arg, network, network);
    }
    ...
    for (const auto& section : args.GetUnrecognizedSections()) {
        warnings += strprintf(_("[%s] is not recognized.\n"), section.m_file, section.m_line, section.m_name);
    }
