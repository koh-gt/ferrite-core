...
The Ferrite Core daemon can be configured to act as a trusted "border
router", implementing the ferrite wire protocol and relay, making
...
The ZeroMQ feature in Ferrite Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building ferrited:
...
For instance:
    ...
    $ ferrited -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/ferrited.tx.raw \
...
These options can also be provided in ferrite.conf.
...
From the perspective of ferrited, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into ferrited directly. Furthermore, no information is
...
using. Ferrited appends an up-counting sequence number to each
